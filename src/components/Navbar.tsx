...
export const Navbar: React.FC<NavbarProps> = ({ userType }) => {
  ...
  const Logo = () => (
    <div className="flex items-center space-x-3">
      ...
      <Link to="/" className="text-xl font-bold text-[#141711]">AyurDiet</Link>
    </div>
  );
      ...
      <>
        ...
        <div className="flex justify-around items-center py-2">
          {[
            ...
            { icon: '🥗', label: 'Diet', path: '/dashboard/patient/diet' },
            { icon: '📊', label: 'Progress', path: '/dashboard/patient/progress' },
            { icon: '💬', label: 'Chat', path: '/dashboard/patient/chat' },
            { icon: '👤', label: 'Profile', path: '/dashboard/patient/profile' }
          ].map((item) => (
            <Link
              key={item.path}
              to={item.path}
  ...
  return (
    <header className="bg-white/90 backdrop-blur-md border-b border-[#E0E5DC] sticky top-0 z-50">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          <Logo />
          
          <div className="hidden md:flex items-center space-x-8">
            <Link to="/login/patient" className="text-[#4B5563] hover:text-[#7FB069] transition-colors">
              Patient Login
            </Link>
            <Link to="/login/dietician" className="text-[#4B5563] hover:text-[#7FB069] transition-colors">
              Dietician Login
            </Link>
          </div>
        </div>
